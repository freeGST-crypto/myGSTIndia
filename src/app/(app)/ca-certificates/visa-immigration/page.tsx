...
const CertificateToPrint = React.forwardRef<HTMLDivElement, { formData: FormData }>(({ formData }, ref) => {
    ...
    const totalImmovable = formData.immovableProperties.reduce((acc, asset) => acc + (Number(asset.value) || 0), 0);
    const totalLiquid = formData.liquidAssets.reduce((acc, asset) => acc + (Number(asset.value) || 0), 0);
    const totalBank = formData.bankBalances.reduce((acc, asset) => acc + (Number(asset.value) || 0), 0);
    const totalIncome = formData.currentIncome.reduce((acc, asset) => acc + (Number(asset.value) || 0), 0);
    const totalLoan = formData.educationLoan.reduce((acc, asset) => acc + (Number(asset.value) || 0), 0);
    const grandTotal = totalImmovable + totalLiquid + totalBank + totalIncome + totalLoan;
    
    const toUsd = (inr: number) => formData.usdRate > 0 ? inr / formData.usdRate : 0;
    ...
    const renderDetailTable = (title: string, data: { description: string, value: number }[], total: number) => {
    ...
    return (
        <div ref={ref} className="prose prose-sm dark:prose-invert max-w-none p-8 leading-relaxed">
            ...
            {renderDetailTable("IMMOVABLE PROPERTIES", formData.immovableProperties, totalImmovable)}
            {renderDetailTable("INVESTMENT PROPERTIES & LIQUID ASSESTS", formData.liquidAssets, totalLiquid)}
            {renderDetailTable("BANK BALANCES", formData.bankBalances, totalBank)}
            {renderDetailTable("CURRENT INCOME", formData.currentIncome, totalIncome)}
            {renderDetailTable("EDUCATIONAL LOAN", formData.educationLoan, totalLoan)}
            ...
            return (
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <FormField control={form.control} name="documentName" render={({ field }) => (<FormItem><FormLabel>Document Name (for saving)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
                        <Separator/>
                    ...
                    <CardFooter className="justify-between">
                        ...
                        <Button type="button" onClick={() => form.trigger().then(isValid => isValid && setStep(2))}>Next <ArrowRight className="ml-2"/></Button>
                    </CardFooter>
                </Card>
            );
        case 2:
